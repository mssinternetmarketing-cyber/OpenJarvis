...
# ─── Counters + Self-update deferred move ────────────────────────────────────
UPDATED=0
SKIPPED=0
UNKNOWN=0
ERRORS=0
...
# When we update ourselves we can't mv while running (bash reads in chunks).
# Set SELF_UPDATE_SRC/DST during processing; this trap moves the file on exit.
_cleanup_self_update() {
    if [ -n "$SELF_UPDATE_SRC" ] && [ -f "$SELF_UPDATE_SRC" ]; then
        mkdir -p "$(dirname "$SELF_UPDATE_DST")" 2>/dev/null
        mv "$SELF_UPDATE_SRC" "$SELF_UPDATE_DST" 2>/dev/null || true
    fi
}
trap _cleanup_self_update EXIT
...
for src_file in "${update_files[@]}"; do
    ...
    if [ "$file_ok" = true ]; then
        log "$fname — deployed"
        UPDATED=$((UPDATED + 1)) || true
        # Move deployed file — defer self-moves until after script exits
        if [ "$DRY_RUN" = false ]; then
            if [ "$fname" = "jarvis_update.sh" ]; then
                SELF_UPDATE_SRC="$src_file"
                SELF_UPDATE_DST="$DEPLOYED_DIR/$fname"
                write_log "MOVED $fname → deployed/$fname (deferred — safe self-update)"
            else
