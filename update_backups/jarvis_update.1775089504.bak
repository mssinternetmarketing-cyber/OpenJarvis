...
# jarvis_update.sh is NOT in DEST_MAP — deploy it manually:
# cp /path/to/jarvis_update.sh $JARVIS_ROOT/jarvis_update.sh && chmod +x $JARVIS_ROOT/jarvis_update.sh
...
UNKNOWN=0
ERRORS=0
# (no self-update trap needed — jarvis_update.sh is deployed manually)
...
for src_file in "${update_files[@]}"; do
    ...
    if [ "$file_ok" = true ]; then
        log "$fname — deployed"
        UPDATED=$((UPDATED + 1)) || true
        # Move deployed file out of updater folder
        if [ "$DRY_RUN" = false ]; then
