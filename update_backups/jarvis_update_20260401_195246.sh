#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════════════════
# jarvis_update.sh — OpenJarvis Intelligent Update Script v2.1
# ═══════════════════════════════════════════════════════════════════════════════
# USAGE:
...
#
# COMMANDS AGENT SUPPORT:
#   Agents can create new tools by writing files to:
#     workspaces/ws_1/commands/scripts/   (.py or .sh tool scripts)
#     workspaces/ws_1/commands/registry/  (command registry JSON)
#   Scripts dropped in OpenJarvisUpdater/ matching commands/*.sh or
#   commands/*.py are auto-deployed to JARVIS_ROOT/commands/ AND ~/bin/
#   making them immediately available as host commands.
#
# ADD NEW FILES:
...
#     COMMAND:name     — copies to commands/ AND installs ~/bin wrapper
# ═══════════════════════════════════════════════════════════════════════════════
...
section "OpenJarvis Updater v2.1 — $(date '+%Y-%m-%d %H:%M:%S')"
...
for src_file in "${update_files[@]}"; do
    ...
    if [ -z "${DEST_MAP[$fname]+_}" ]; then
        ...
        # Rule 4: Python files -> JARVIS_ROOT
        if [[ "$fname" == *.py ]]; then
...
if [ "$DRY_RUN" = false ]; then
    ...
    echo ""
    echo -e "  To create a new agent tool:"
    echo -e "  Name your script ${CYAN}cmd_<toolname>.sh${NC} or ${CYAN}cmd_<toolname>.py${NC}"
    echo -e "  Drop it in $UPDATER_DIR"
    echo -e "  Run ${CYAN}qcai-update${NC} — it becomes a host command automatically"
fi
